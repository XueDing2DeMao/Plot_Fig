...
- Create: `packages/figure-migrations/src/load-support.ts`
...
- File-gate RED/GREEN on 2026-09-02: after folding the line-limit assertion into the existing `typecheck-config.test.ts` case, `pnpm vitest run packages/figure-migrations/src/typecheck-config.test.ts` exited 1 with `[{ "file": "load-internals.ts", "lines": 307 }]`, proving the post-Prettier source tree still violated the 300-line limit. After extracting diagnostics / canonical-clone / reflection helpers into the new private module `packages/figure-migrations/src/load-support.ts`, rerunning the same focused test exited 0 with `1` file passed and `1` test passed.
- Recursive-coverage RED/GREEN on 2026-09-02: after tightening the existing `typecheck-config.test.ts` case to require recursive `src/**/*.ts` coverage and explicitly include `migrations/v0.1.0-to-v1.0.0.ts` plus `migrations/v0.1.0-to-v1.0.0.test.ts`, `pnpm vitest run packages/figure-migrations/src/typecheck-config.test.ts` exited 1 because the flat top-level scan returned only `index.test.ts`, `index.ts`, `load-current.test.ts`, `load-failures.test.ts`, `load-internals.ts`, `load-safety.test.ts`, `load-support.ts`, `load.ts`, `registry.test.ts`, `registry.ts`, `typecheck-config.test.ts`, `types.ts`, `version.test.ts`, and `version.ts`. After replacing that flat scan with a recursive helper, rerunning the same focused command exited 0 with `1` file passed and `1` test passed.
- Split-file gate on 2026-09-02: final PowerShell `Get-ChildItem packages/figure-migrations/src -Recurse -Filter *.ts | Sort-Object FullName | ForEach-Object { $rel = $_.FullName.Replace((Resolve-Path 'packages/figure-migrations/src').Path + '\', '').Replace('\','/'); [pscustomobject]@{ file = $rel; lines = (Get-Content -LiteralPath $_.FullName).Count } }` reported `index.test.ts = 86`, `index.ts = 2`, `load-current.test.ts = 165`, `load-failures.test.ts = 183`, `load-internals.ts = 242`, `load-safety.test.ts = 75`, `load-support.ts = 73`, `load.ts = 11`, `migrations/v0.1.0-to-v1.0.0.test.ts = 169`, `migrations/v0.1.0-to-v1.0.0.ts = 72`, `registry.test.ts = 71`, `registry.ts = 58`, `typecheck-config.test.ts = 119`, `types.ts = 28`, `version.test.ts = 135`, and `version.ts = 120`; every production/test TypeScript file under `packages/figure-migrations/src` is now `<= 300` after final Prettier.
