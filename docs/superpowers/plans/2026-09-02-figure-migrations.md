...
- Split-file gate on 2026-09-02: final PowerShell `Get-ChildItem packages/figure-migrations/src -Recurse -Filter *.ts | Sort-Object FullName | ForEach-Object { $rel = $_.FullName.Replace((Resolve-Path 'packages/figure-migrations/src').Path + '\', '').Replace('\','/'); [pscustomobject]@{ file = $rel; lines = (Get-Content -LiteralPath $_.FullName).Count } }` reported `index.test.ts = 86`, `index.ts = 2`, `load-current.test.ts = 165`, `load-failures.test.ts = 183`, `load-internals.ts = 242`, `load-safety.test.ts = 75`, `load-support.ts = 73`, `load.ts = 11`, `migrations/v0.1.0-to-v1.0.0.test.ts = 169`, `migrations/v0.1.0-to-v1.0.0.ts = 72`, `registry.test.ts = 71`, `registry.ts = 58`, `typecheck-config.test.ts = 114`, `types.ts = 28`, `version.test.ts = 135`, and `version.ts = 120`; every production/test TypeScript file under `packages/figure-migrations/src` is now `<= 300` after final Prettier.
