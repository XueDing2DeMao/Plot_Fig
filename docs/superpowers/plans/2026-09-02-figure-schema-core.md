...
- Final evidence refresh: after `pnpm build`, running `node --input-type=module -e 'import("@plot-fig/figure-schema") ...'` from the `packages/figure-schema` working directory exited 0 with `package-entry self-import smoke ok`, proving the published package name resolves through `package.json#exports` to the exact 3-key runtime surface and that `validateFigureTemplate`, `validateFigureDocument`, and `canonicalizeFigurePayload` are callable through the real package entrypoint.
- Final evidence refresh gates: `pnpm format:check`, `pnpm typecheck`, and `pnpm schema:check` were rerun fresh alongside that self-import smoke and each exited 0.
